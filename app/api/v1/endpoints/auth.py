...
async def login(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    **Returns:**
    - JWT access token (expires in 30 minutes by default)
    - Token type: "bearer"
...
async def login_json(credentials: UserLogin):
    ...
    **Note:**
    This endpoint is useful for applications that prefer JSON over form data.
    The standard `/login` endpoint uses OAuth2 form data format.
    """
    # Authenticate user
...
async def refresh_token(current_user: User = Depends(get_current_active_user)):
    ...
    **Authentication Required:**
    Provide your current (valid) access token.
    ...
    **Use Case:**
    Call this endpoint before your token expires to get a new one
...
async def logout(current_user: User = Depends(get_current_active_user)):
    ...
    **For Production:**
    ...
    - Token blacklisting (store revoked tokens)
    - Short-lived tokens with refresh tokens
    - Redis-based token management
    
    **Returns:**
    - Success message
    - Instructions to discard the token
    """
...
async def test_authentication(current_user: User = Depends(get_current_active_user)):
    ...
    **Use Case:**
    Use this to verify your token is valid and authentication is working correctly.
    """
    return {
